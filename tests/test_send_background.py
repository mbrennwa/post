...
class RunOutboundSendTests(unittest.TestCase):
    ...
    @mock.patch("post.compose_window.threading.Thread", ImmediateThread)
    @mock.patch("post.compose_window.GLib.idle_add", side_effect=_run_idle_add)
    def test_success_sets_message_sent(self, _idle_add, _persist) -> None:
        ...
        self.mail.send_outbound_queue_item.assert_called_once_with("queue-1")
        self.assertEqual(self.status_messages, ["Message sent"])

    @mock.patch("post.compose_window.persist_outbound_send", return_value="queue-1")
    @mock.patch("post.compose_window.threading.Thread", ImmediateThread)
    @mock.patch("post.compose_window.GLib.idle_add", side_effect=_run_idle_add)
    def test_send_queued_updates_status_and_outbox(self, _idle_add, _persist) -> None:
        self.mail.send_outbound_queue_item.side_effect = SendQueued(MESSAGE_QUEUED)
        outbox_changed = mock.Mock()
    ...
    def test_error_shows_toast_on_parent(
        self, _idle_add, _persist, show_error_toast
    ) -> None:
        self.mail.send_outbound_queue_item.side_effect = SendError("SMTP failed")
        parent = mock.Mock()
        outbox_changed = mock.Mock()
    ...
    @mock.patch("post.compose_window.threading.Thread", ImmediateThread)
    @mock.patch("post.compose_window.GLib.idle_add", side_effect=_run_idle_add)
    def test_success_deletes_draft_before_status(self, _idle_add, _persist) -> None:
    ...
    def test_persist_failure_shows_toast(self, persist, show_error_toast) -> None:
        parent = mock.Mock()
        run_outbound_send(
            mail=self.mail,
            parent=parent,
        ...
        self.mail.send_outbound_queue_item.assert_not_called()
