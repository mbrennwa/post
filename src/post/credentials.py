...
"""Password prompt for mail accounts without stored credentials."""
...
_PROMPT_BODY_DEFAULT = "Enter the password for this email account."
_PROMPT_BODY_BY_REASON: dict[PasswordPromptReason, str] = {
    "check_mail": "Needed to check mail for this account.",
    "send_mail": "Needed to send a message in Outbox.",
}


def prompt_password_sync(
    parent: PasswordPrompt,
    account_label: str,
    *,
    reason: PasswordPromptReason | None = None,
) -> str | None:
    """Show a modal password dialog; safe to call from a worker thread."""
    if threading.current_thread() is threading.main_thread():
        return _show_password_dialog(parent, account_label, reason=reason)

    result: dict[str, str | None] = {"password": None}
    done = threading.Event()

    def on_idle() -> bool:
        result["password"] = _show_password_dialog(
            parent, account_label, reason=reason
        )
        done.set()
        return False
...
def _show_password_dialog(
    ...
    *,
    ...
    parent.present()
    log.info(
        "Showing password sign-in dialog for %s (reason=%s)",
        account_label,
    ...
    body = (
        _PROMPT_BODY_BY_REASON[reason]
        if reason in _PROMPT_BODY_BY_REASON
        else _PROMPT_BODY_DEFAULT
    )
