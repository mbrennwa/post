...
There is no supported helpers-off mode in the UI.
...
- **`post.mail.camel_runtime`** — UI supervisor: spawn / call / watchdog timeout → **kill** / respawn; `CamelRuntimePool` keyed by account. Helpers set `POST_MAIL_CAMEL_HELPER_PROCESS=1` so they do not nest.
...
13. **Folder transfer / Archive (#189/#404/#462/#461)** — Interactive Archive/Trash/move/seen/flagged are **local-first**: update the list + folder-index immediately, persist a `QueuedOperation`, apply local Camel state via a short helper RPC, and return without waiting on Graph/IMAP MOVE. Sidebar bulk Archive (All / Read / Read+Unflagged) selects UIDs from helper `get_folder_messages` (never UI-only cache) then calls the same local-first `archive_messages` (#461). Server work flushes **serially per account through the Camel helper** (`flush_account_operation_queue`) — never against the UI process Evolution dirs. Coalesce consecutive same-folder batches; never toast “try again” for a busy transfer. `transfer_messages_to_sync` still uses a finite `Gio.Cancellable` timeout; soft-succeed when source UIDs are already gone. After move (including soft-succeed), prune Camel `FolderSummary` UIDs locally (Evolution-style) and update Post’s folder-index cache with **destination RestIds only**. Do **not** park Inbox RestIds as openable Archive rows. For a small interactive move, a **bounded** dest `refresh_info` (8s) plus per-UID `synchronize_message_sync` learns dest ids; do **not** block UI completion on unbounded Graph `refresh_info_sync` of a 29k Archive. For `microsoft365` / `ews` bulk, skip post-transfer full-folder `synchronize_sync` / `refresh_info_sync` (#189). Account transfer-busy / not-responding badges still escalate on timeout during flush; do **not** refuse new interactive commands — queue them. Pending local removals must not reappear on Inbox refresh until flush completes (#261). **Quit** waits for in-flight Archive/move/trash, the operation queue, and outbound send via async gates + `shutdown_async` so the GTK loop keeps pumping (#443); do not call `shutdown_sync` from GTK. **If Graph ignores cancel:** terminate that account’s **helper process** (#437); do not add a second in-process Camel session in the UI.
