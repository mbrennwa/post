...
6. **Offline body download** — `OfflineBodySyncCoordinator` runs `downsync_sync` on the mail I/O thread only. Each folder downsync is bound by a **30s** per-folder `Gio.Cancellable` timeout so one slow folder cannot pin `post-mail-io` for minutes (#197); a folder timeout does **not** cancel the rest of the account pass (#208). Folders are ordered ordinary → Archive → Trash → Junk. Interactive work preempts offline sync via `cancel_all()` and between-folder yield. See [offline-body-cache.md](offline-body-cache.md).
